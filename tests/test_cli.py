# -*- coding: utf-8 -*-
from flasktest.models import Admin, Post, Category, Comment
from flasktest.extensions import db
...
class CLITestCase(BaseTestCase):
    ...
    def test_init_command(self):
        result = self.runner.invoke(args=['init', '--username', 'tong', '--password', '123'])
        ...
        self.assertEqual(Admin.query.first().username, 'tong')
        self.assertEqual(Category.query.first().name, 'Default')

    def test_init_command_with_update(self):
        self.runner.invoke(args=['init', '--username', 'tong', '--password', '123'])
        result = self.runner.invoke(args=['init', '--username', 'new tong', '--password', '123'])
        ...
        self.assertEqual(Admin.query.count(), 1)
        self.assertEqual(Admin.query.first().username, 'new tong')
        self.assertEqual(Category.query.first().name, 'Default')
